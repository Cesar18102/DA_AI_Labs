...
{
    public class RectBorderBuilder : BorderBuilder
    {
        public RectBorderBuilder(float width, Color color, bool randColor = false) : base(width, color, randColor) { }

        public override void Draw(Graphics canvas, int ltx, int lty, int width, int height, string sign = "")
        {
            canvas.DrawRectangle(pen, ltx - pen.Width / 2, lty - pen.Width / 2, width + pen.Width, height + pen.Width);
